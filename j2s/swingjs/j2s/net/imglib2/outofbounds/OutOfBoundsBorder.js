...
/*c*/var C$=Clazz.newClass(P$, "OutOfBoundsBorder", null, 'net.imglib2.AbstractLocalizable', 'net.imglib2.outofbounds.OutOfBounds');
...
C$.$fields$=[['Z',['isOutOfBounds'],'O',['outOfBoundsRandomAccess','net.imglib2.RandomAccess','min','long[]','+beforeMin','+max','+pastMax','dimIsOutOfBounds','boolean[]']]]

Clazz.newMeth(C$, ['c$$net_imglib2_outofbounds_OutOfBoundsBorder'], function (outOfBounds) {
...
return Clazz.new_(1,{T:"Object"},C$.c$$net_imglib2_outofbounds_OutOfBoundsBorder,[this]);
});
...
this.checkOutOfBounds$();
 ...
 else if (!this.dimIsOutOfBounds[d]) this.outOfBoundsRandomAccess.fwd$I(d);
});
...
this.dimIsOutOfBounds[d]=false;
this.checkOutOfBounds$();
} else if (!this.dimIsOutOfBounds[d]) this.outOfBoundsRandomAccess.bck$I(d);
});
...
var maxD=this.max[d];
if (position < minD) {
this.outOfBoundsRandomAccess.setPosition$J$I(minD, d);
this.dimIsOutOfBounds[d]=this.isOutOfBounds=true;
} else if (position > maxD) {
this.outOfBoundsRandomAccess.setPosition$J$I(maxD, d);
this.dimIsOutOfBounds[d]=this.isOutOfBounds=true;
} else {
