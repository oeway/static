...
C$.addMethod$java_util_Map$reflect_Method(sigs, methods[i]);
}
} else {
for (var i=0; i < methods.length; i++) {
...
Clazz.newMeth(C$, 'addMethod$java_util_Map$reflect_Method', function (sigs, method) {
var signature=Clazz.new_($I$(5,1).c$$reflect_Method,[method]);
if (!sigs.containsKey$O(signature)) {
sigs.put$TK$TV(signature, method);
...
var t=P$.MethodUtil.getTrampolineClass$();
...
throw Clazz.new_($I$(6,1).c$$S$Throwable,["bouncer cannot be found", e]);
} else {
throw e;
}
}
...
var cs=Clazz.new_($I$(8,1).c$$java_net_URL$java_security_cert_CertificateA,[null, null]);
...
var $in=Clazz.new_($I$(9,1).c$$java_io_InputStream,[uc.getInputStream$()]);
var b;
try {
b=$I$(10).readFully$java_io_InputStream$I$Z($in, len, true);
} finally {
$in.close$();
}
return b;
...
perms.add$java_security_Permission(Clazz.new_($I$(11,1)));
return perms;
});
...
Clazz.newMeth(C$, 'c$$reflect_Method', function (m) {
;C$.$init$.apply(this);
