...
var result=Clazz.new_(1,{E:"Object"},$I$(2,1));
var direct=annotations.get$O(annoClass);
if (direct != null ) result.add$TE(direct);
...
result.addAll$I$java_util_Collection((indirectFirst ? 0 : 1), $I$(3).asList$TTA(indirect));
}var arr=Clazz.array(annoClass, result.size$());
return result.toArray$TTA(arr);
}, 1);
...
var valueArray=C$.getValueArray$annotation_Annotation(container);
C$.checkTypes$TAA$annotation_Annotation$Class(valueArray, container, annoClass);
return valueArray;
}, 1);
...
Clazz.newMeth(C$, 'getValueArray$annotation_Annotation', function (container) {
try {
var containerClass=container.annotationType$();
var annoType=$I$(7).getInstance$Class(containerClass);
if (annoType == null ) throw C$.invalidContainerException$annotation_Annotation$Throwable(container, null);
var m=annoType.members$().get$O("value");
if (m == null ) throw C$.invalidContainerException$annotation_Annotation$Throwable(container, null);
m.setAccessible$Z(true);
var values=m.invoke$O$OA(container, []);
return values;
} catch (e) {
...
throw C$.invalidContainerException$annotation_Annotation$Throwable(container, e);
} else {
throw e;
}
}
...
Clazz.newMeth(C$, 'invalidContainerException$annotation_Annotation$Throwable', function (anno, cause) {
return Clazz.new_($I$(8,1).c$$S$Throwable,[anno + " is an invalid container for repeating annotations", cause]);
}, 1);

Clazz.newMeth(C$, 'checkTypes$TAA$annotation_Annotation$Class', function (annotations, container, annoClass) {
for (var a, $a = 0, $$a = annotations; $a<$$a.length&&((a=($$a[$a])),1);$a++) {
if (!annoClass.isInstance$O(a)) {
throw Clazz.new_($I$(8,1).c$$S,[String.format$S$OA("%s is an invalid container for repeating annotations of type: %s", [container, annoClass])]);
}}
}, 1);
