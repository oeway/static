(function(){var P$=Clazz.newPackage("sun.misc"),I$=[[0,'java.util.Hashtable','sun.misc.SignalHandler','sun.misc.NativeSignalHandler','Thread']],$I$=function(i,n){return(i=(I$[i]||(I$[i]=Clazz.load(I$[0][i])))),!n&&i.$load$&&Clazz.load(i,2),i};
/*c*/var C$=Clazz.newClass(P$, "Signal");

C$.$clinit$=2;

C$.$static$ = function() {C$.$static$=0;
C$.handlers=Clazz.new_(1,{K:"sun.misc.Signal",V:"sun.misc.SignalHandler"},$I$(1,1).c$$I,[4]);
C$.signals=Clazz.new_(1,{K:"Integer",V:"sun.misc.Signal"},$I$(1,1).c$$I,[4]);
}
...
}C$.signals.put$TK$TV(new Integer(sig.number), sig);
{
var oldHandler=C$.handlers.get$O(sig);
C$.handlers.remove$O(sig);
if (newH == 2) {
C$.handlers.put$TK$TV(sig, handler);
}if (oldH == 0) {
return $I$(2).SIG_DFL;
} else if (oldH == 1) {
return $I$(2).SIG_IGN;
...
var sig=C$.signals.get$O(new Integer(number));
...
this.$finals$.handler.handle$(this.$finals$.sig);
});
})()
