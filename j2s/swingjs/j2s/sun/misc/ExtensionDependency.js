(function(){var P$=Clazz.newPackage("sun.misc"),p$1={},I$=[[0,'java.util.Vector',['java.util.jar.Attributes','.Name'],'java.util.StringTokenizer','sun.misc.ExtensionInfo','java.security.AccessController','java.util.jar.JarFile','sun.misc.Launcher','java.io.File','sun.security.action.GetPropertyAction','sun.misc.JarFilter','sun.net.www.ParseUtil','Boolean']],$I$=function(i,n){return(i=(I$[i]||(I$[i]=Clazz.load(I$[0][i])))),!n&&i.$load$&&Clazz.load(i,2),i};
...
if (C$.providers == null ) {
C$.providers=Clazz.new_(1,{E:"sun.misc.ExtensionInstallationProvider"},$I$(1,1));
}C$.providers.add$TE(eip);
}, 1);
...
try {
var fExtension;
var dirs=P$.ExtensionDependency.getExtDirs$();
for (var i=0; i < dirs.length; i++) {
for (var j=0; j < this.$finals$.fileExt.length; j++) {
if (this.$finals$.extName.toLowerCase$().endsWith$S(this.$finals$.fileExt[j])) {
fExtension=Clazz.new_($I$(8,1).c$$java_io_File$S,[dirs[i], this.$finals$.extName]);
} else {
fExtension=Clazz.new_($I$(8,1).c$$java_io_File$S,[dirs[i], this.$finals$.extName + this.$finals$.fileExt[j]]);
}P$.ExtensionDependency.debug$S("checkExtensionExists:fileName " + fExtension.getName$());
...
var s=$I$(5).doPrivileged$java_security_PrivilegedAction(Clazz.new_($I$(9,1).c$$S,["java.ext.dirs"]));
var dirs;
if (s != null ) {
var st=Clazz.new_($I$(3,1).c$$S$S,[s, "/"]);
var count=st.countTokens$();
C$.debug$S("getExtDirs count " + count);
dirs=Clazz.array($I$(8), [count]);
for (var i=0; i < count; i++) {
dirs[i]=Clazz.new_($I$(8,1).c$$S,[st.nextToken$()]);
C$.debug$S("getExtDirs dirs[" + i + "] " + dirs[i] );
}
} else {
dirs=Clazz.array($I$(8), [0]);
...
Clazz.newMeth(C$, 'run$', function () {
try {
return P$.ExtensionDependency.getExtFiles$java_io_FileA(P$.ExtensionDependency.getExtDirs$());
} catch (e) {
if (Clazz.exceptionOf(e,"java.io.IOException")){
P$.ExtensionDependency.debug$S("Cannot get list of installed extensions");
p$1.debugException$Throwable.apply(this.b$['sun.misc.ExtensionDependency'], [e]);
