...
while ((pos=cp.indexOf$S$I("/", lastPos)) != -1){
if (pos - lastPos > 0) {
path[count++]=Clazz.new_($I$(7,1).c$$S,[cp.substring$I$I(lastPos, pos)]);
} else {
path[count++]=Clazz.new_($I$(7,1).c$$S,["."]);
}lastPos=pos + 1;
}
if (lastPos < cp.length$()) {
path[count++]=Clazz.new_($I$(7,1).c$$S,[cp.substring$I(lastPos)]);
} else {
path[count++]=Clazz.new_($I$(7,1).c$$S,["."]);
}if (count != maxCount) {
...
$I$(5).getJavaNetAccess$().getURLClassPath$(this).initLookupCache$ClassLoader(this);
}, 1);
...
dirs[i]=Clazz.new_($I$(7,1).c$$S,[st.nextToken$()]);
}
} else {
...
var urls=Clazz.new_(1,{E:"java.net.URL"},$I$(8,1));
...
urls.add$TE(P$.Launcher.getFileURL$java_io_File(f));
}}
}}
...
var dir=Clazz.new_($I$(7,1).c$$S,[urls[i].getPath$()]).getParentFile$();
if (dir != null  && !dir.equals$O(prevDir) ) {
var arch=$I$(10).getSavedProperty$S("os.arch");
if (arch != null ) {
var file=Clazz.new_($I$(7,1).c$$java_io_File$S,[Clazz.new_($I$(7,1).c$$java_io_File$S,[dir, arch]), name]);
if (file.exists$()) {
...
var domain=Clazz.new_($I$(12,1).c$$java_security_CodeSource$java_security_PermissionCollection,[Clazz.new_($I$(13,1).c$$java_net_URL$java_security_cert_CertificateA,[perms.getCodeBase$(), null]), perms]);
var acc=Clazz.new_($I$(14,1).c$$java_security_ProtectionDomainA,[Clazz.array($I$(12), -1, [domain])]);
return acc;
}, 1);
...
var path=(s == null ) ? Clazz.array($I$(7), [0]) : P$.Launcher.getClassPath$S(s);
...
Clazz.newMeth(C$, 'run$', function () {
var urls=(this.$finals$.s == null ) ? Clazz.array($I$(9), [0]) : P$.Launcher.pathToURLs$java_io_FileA(this.$finals$.path);
...
this.$ucp=$I$(5).getJavaNetAccess$().getURLClassPath$(this);
...
C$.superclazz.prototype.addURL$java_net_URL.apply(this, [P$.Launcher.getFileURL$java_io_File(Clazz.new_($I$(7,1).c$$S,[path]))]);
}, p$1);
...
Clazz.newMeth(C$, 'run$', function () {
var classPath=P$.Launcher.getClassPath$S($I$(4).bootClassPath);
var len=classPath.length;
var seenDirs=Clazz.new_(1,{E:"java.io.File"},$I$(18,1));
for (var i=0; i < len; i++) {
var curEntry=classPath[i];
if (!curEntry.isDirectory$()) {
curEntry=curEntry.getParentFile$();
}if (curEntry != null  && seenDirs.add$TE(curEntry) ) {
$I$(2).registerDirectory$java_io_File(curEntry);
}}
return P$.Launcher.pathToURLs$java_io_FileA(classPath);
});
})()
), Clazz.new_(P$.Launcher$BootClassPathHolder$1.$init$, [this, null])));
} else {
urls=Clazz.array($I$(9), [0]);
}C$.bcp=Clazz.new_($I$(19,1).c$$java_net_URLA$java_net_URLStreamHandlerFactory,[urls, $I$(4).factory]);
