...
curves.add$TE(Clazz.new_($I$(1,1).c$$D$D,[x, y]));
}, 1);

Clazz.newMeth(C$, 'insertLine$java_util_Vector$D$D$D$D', function (curves, x0, y0, x1, y1) {
if (y0 < y1 ) {
curves.add$TE(Clazz.new_($I$(2,1).c$$D$D$D$D$I,[x0, y0, x1, y1, 1]));
} else if (y0 > y1 ) {
curves.add$TE(Clazz.new_($I$(2,1).c$$D$D$D$D$I,[x1, y1, x0, y0, -1]));
} else {
}}, 1);
...
throw Clazz.new_($I$(5,1).c$$S,["sun.awt.geom.Curve backstepping from " + new Double(yrange[0]).toString() + " to " + new Double(y1).toString() ]);
}yrange[1]=y1;
