(function(){var P$=Clazz.newPackage("io.scif.img.cell.loaders"),I$=[[0,'java.nio.ByteBuffer','java.nio.ByteOrder','io.scif.util.FormatTools','net.imglib2.img.basictypeaccess.array.DoubleArray','net.imglib2.type.numeric.real.DoubleType']],$I$=function(i,n){return(i=(I$[i]||(I$[i]=Clazz.load(I$[0][i])))),!n&&i.$load$&&Clazz.load(i,2),i};
/*c*/var C$=Clazz.newClass(P$, "DoubleArrayLoader", null, 'io.scif.img.cell.loaders.AbstractArrayLoader');
...
Clazz.newMeth(C$, ['convertBytes$net_imglib2_img_basictypeaccess_array_DoubleArray$BA$I','convertBytes$TA$BA$I'], function (data, bytes, planesRead) {
...
bb.asDoubleBuffer$().get$DA$I$I(data.getCurrentStorageArray$(), offset, (bytes.length/bpp|0));
} else {
var pixelType=iMeta.getPixelType$();
var bpp=$I$(3).getBytesPerPixel$I(pixelType);
var offset=planesRead * ((bytes.length/bpp|0));
for (var index=0; index < (bytes.length/bpp|0); index++) {
var value=this.utils$().decodeWord$BA$I$I$Z(bytes, index * bpp, pixelType, iMeta.isLittleEndian$());
data.setValue$I$D(offset + index, value);
}
}});
