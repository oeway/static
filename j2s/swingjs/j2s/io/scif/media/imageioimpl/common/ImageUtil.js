(function(){var P$=Clazz.newPackage("io.scif.media.imageioimpl.common"),I$=[[0,'java.awt.color.ColorSpace','io.scif.media.imageioimpl.common.BogusColorSpace','java.awt.image.ComponentColorModel','java.awt.image.DirectColorModel','java.awt.image.IndexColorModel','io.scif.media.imageioimpl.common.I18N','java.awt.image.DataBuffer','javax.imageio.ImageTypeSpecifier','javax.imageio.spi.ImageReaderSpi','javax.imageio.spi.ImageWriterSpi','java.util.Locale','java.util.ArrayList']],$I$=function(i,n){return(i=(I$[i]||(I$[i]=Clazz.load(I$[0][i])))),!n&&i.$load$&&Clazz.load(i,2),i};
...
map[i]=((((i * 255/(numEntries - 1)|0))|0)|0);
}
...
while (xRemaining > 8){
var datum=data[i++];
binaryDataArray[b++]=((((datum >>> 8) & 255)|0)|0);
binaryDataArray[b++]=(((datum & 255)|0)|0);
xRemaining-=16;
}
if (xRemaining > 0) {
binaryDataArray[b++]=((((data[i] >>> 8) & 255)|0)|0);
}eltOffset+=lineStride;
}
...
while (xRemaining > 24){
var datum=data[i++];
binaryDataArray[b++]=((((datum >>> 24) & 255)|0)|0);
binaryDataArray[b++]=((((datum >>> 16) & 255)|0)|0);
binaryDataArray[b++]=((((datum >>> 8) & 255)|0)|0);
binaryDataArray[b++]=(((datum & 255)|0)|0);
xRemaining-=32;
}
var shift=24;
while (xRemaining > 0){
binaryDataArray[b++]=((((data[i] >>> shift) & 255)|0)|0);
shift-=8;
xRemaining-=8;
}
eltOffset+=lineStride;
...
binaryDataArray[b++]=(((((data[i++] & 255) << leftShift) | ((data[i] & 255) >>> rightShift))|0)|0);
} else {
binaryDataArray[b++]=((((data[i] & 255) << leftShift)|0)|0);
}xRemaining-=8;
}
eltOffset+=lineStride;
}
...
if (mod <= 8) {
binaryDataArray[b++]=(((left >>> (8 - mod))|0)|0);
} else {
var delta=mod - 8;
var right=data[i + 1] & 65535;
binaryDataArray[b++]=((((left << delta) | (right >>> (16 - delta)))|0)|0);
}}
eltOffset+=lineStride;
}
} else if (Clazz.instanceOf(dataBuffer, "java.awt.image.DataBufferInt")) {
...
var left=data[i];
if (mod <= 24) {
binaryDataArray[b++]=(((left >>> (24 - mod))|0)|0);
} else {
var delta=mod - 24;
var right=data[i + 1];
binaryDataArray[b++]=((((left << delta) | (right >>> (32 - delta)))|0)|0);
}}
eltOffset+=lineStride;
}
...
var b=($b$[0] = data[(bOffset/8|0)], $b$[0]);
bdata[k++]=((((b >>> (7 - bOffset & 7)) & 1)|0)|0);
bOffset++;
}
eltOffset+=lineStride;
}
...
var xRemaining=rectWidth;
while (xRemaining > 0){
var datum=($b$[0] = binaryDataArray[b++], $b$[0]);
if (xRemaining > leftShift8) {
data[i]=((((data[i] & mask) | ((datum & 255) >>> rightShift))|0)|0);
data[++i]=((((datum & 255) << leftShift)|0)|0);
} else if (xRemaining > leftShift) {
data[i]=((((data[i] & mask) | ((datum & 255) >>> rightShift))|0)|0);
i++;
data[i]=((((data[i] & mask1) | ((datum & 255) << leftShift))|0)|0);
} else {
var remainMask=(1 << leftShift - xRemaining) - 1;
data[i]=((((data[i] & (mask | remainMask)) | (datum & 255) >>> rightShift & ~remainMask)|0)|0);
}xRemaining-=8;
}
eltOffset+=lineStride;
}
...
if (bdata[k++] != 0) {
data[$k$=(bOffset/8|0)]=(data[$k$]|(((1 << (7 - bOffset & 7))|0))|0);
}bOffset++;
}
eltOffset+=lineStride;
}
...
colorModel=Clazz.new_($I$(4,1).c$$java_awt_color_ColorSpace$I$I$I$I$I$Z$I,[colorSpace, bits, rmask, gmask, bmask, amask, false, sampleModel.getDataType$()]);
} else if (Clazz.instanceOf(sampleModel, "java.awt.image.MultiPixelPackedSampleModel")) {
var bits=(sampleModel).getPixelBitStride$();
var size=1 << bits;
var comp=Clazz.array(Byte.TYPE, [size]);
for (var i=0; i < size; i++) comp[i]=((((255 * i/(size - 1)|0))|0)|0);
...
Clazz.newMeth(C$, 'isGrayscaleMapping$java_awt_image_IndexColorModel', function (icm) {
if (icm == null ) {
throw Clazz.new_(Clazz.load('IllegalArgumentException').c$$S,["icm == null!"]);
...
icm.getGreens$BA(g);
icm.getBlues$BA(b);
var isGrayToColor=true;
for (var i=0; i < mapSize; i++) {
var temp=($b$[0] = (((i * 255/(mapSize - 1)|0))|0), $b$[0]);
...
if (!isGrayToColor) {
isGrayToColor=true;
for (var i=0, j=mapSize - 1; i < mapSize; i++, j--) {
var temp=($b$[0] = (((j * 255/(mapSize - 1)|0))|0), $b$[0]);
if (r[i] != temp || g[i] != temp  || b[i] != temp ) {
isGrayToColor=false;
break;
}}
...
Clazz.newMeth(C$, 'getDestinationType$javax_imageio_ImageReadParam$java_util_Iterator', function (param, imageTypes) {
...
Clazz.newMeth(C$, 'isNonStandardICCColorSpace$java_awt_color_ColorSpace', function (cs) {
var retval=false;
try {
retval=(Clazz.instanceOf(cs, "java.awt.color.ICC_ColorSpace")) && !(cs.isCS_sRGB$() || cs.equals$O($I$(1).getInstance$I(1004)) || cs.equals$O($I$(1).getInstance$I(1003)) || cs.equals$O($I$(1).getInstance$I(1001)) || cs.equals$O($I$(1).getInstance$I(1002))  ) ;
} catch (e) {
...
Clazz.newMeth(C$, 'getJDKImageReaderWriterSPI$javax_imageio_spi_ServiceRegistry$S$Z', function (registry, formatName, isReader) {
var iioRegistry=registry;
var spiClass;
var descPart;
if (isReader) {
spiClass=Clazz.getClass($I$(9));
descPart=" image reader";
} else {
spiClass=Clazz.getClass($I$(10));
descPart=" image writer";
}var iter=iioRegistry.getServiceProviders$Class$Z(spiClass, true);
var formatNames;
var provider;
var desc="standard " + formatName + descPart ;
var jiioPath="io.scif.media.imageioimpl";
var locale=$I$(11).getDefault$();
var list=Clazz.new_($I$(12,1));
while (iter.hasNext$()){
provider=iter.next$();
if (provider.getVendorName$().startsWith$S("Sun Microsystems") && desc.equalsIgnoreCase$S(provider.getDescription$java_util_Locale(locale)) && !provider.getPluginClassName$().startsWith$S(jiioPath)  ) {
formatNames=provider.getFormatNames$();
for (var i=0; i < formatNames.length; i++) {
if (formatNames[i].equalsIgnoreCase$S(formatName)) {
...
Clazz.newMeth(C$, 'processOnRegistration$javax_imageio_spi_ServiceRegistry$Class$S$javax_imageio_spi_ImageReaderWriterSpi$I$I', function (registry, category, formatName, spi, deregisterJvmVersion, priorityJvmVersion) {
var jvmVendor=System.getProperty$S("java.vendor");
var jvmVersionString=System.getProperty$S("java.specification.version");
var verIndex=jvmVersionString.indexOf$S("1.");
jvmVersionString=jvmVersionString.substring$I(verIndex + 2);
var jvmVersion=Integer.parseInt$S(jvmVersionString);
if (jvmVendor.equals$O("Sun Microsystems Inc.")) {
var list;
if (Clazz.instanceOf(spi, "javax.imageio.spi.ImageReaderSpi")) list=C$.getJDKImageReaderWriterSPI$javax_imageio_spi_ServiceRegistry$S$Z(registry, formatName, true);
 else list=C$.getJDKImageReaderWriterSPI$javax_imageio_spi_ServiceRegistry$S$Z(registry, formatName, false);
if (jvmVersion >= deregisterJvmVersion && list.size$() != 0 ) {
registry.deregisterServiceProvider$TT$Class(spi, category);
} else {
for (var i=0; i < list.size$(); i++) {
if (jvmVersion >= priorityJvmVersion) {
registry.setOrdering$Class$TT$TT(category, list.get$I(i), spi);
} else {
registry.setOrdering$Class$TT$TT(category, spi, list.get$I(i));
}}
}}}, 1);
