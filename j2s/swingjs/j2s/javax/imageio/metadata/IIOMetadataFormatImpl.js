...
this.elementMap.put$TK$TV(rootName, root);
}, 1);
...
parent.childList.add$TE(elementName);
element.parentList.add$TE(parentName);
this.elementMap.put$TK$TV(elementName, element);
});
...
element.attrList.add$TE(attrName);
element.attrMap.put$TK$TV(attrName, attr);
});
...
Clazz.newMeth(C$, 'addObjectValue$S$Class$Z$TT', function (elementName, classType, required, defaultValue) {
var element=p$1.getElement$S.apply(this, [elementName]);
...
Clazz.newMeth(C$, 'addObjectValue$S$Class$Z$TT$java_util_List', function (elementName, classType, required, defaultValue, enumeratedValues) {
var element=p$1.getElement$S.apply(this, [elementName]);
if (enumeratedValues == null ) {
throw Clazz.new_(Clazz.load('IllegalArgumentException').c$$S,["enumeratedValues == null!"]);
}if (enumeratedValues.size$() == 0) {
...
Clazz.newMeth(C$, 'addObjectValue$S$Class$TT$Comparable$Comparable$Z$Z', function (elementName, classType, defaultValue, minValue, maxValue, minInclusive, maxInclusive) {
var element=p$1.getElement$S.apply(this, [elementName]);
var obj=Clazz.new_($I$(5,1), [this, null]);
obj.valueType=2;
if (minInclusive) {
...
return null;
}return element.childList.toArray$TTA(Clazz.array(String, [0]));
});
...
return names.toArray$TTA(result);
});
...
return values.toArray$TTA(result);
});
...
return vlist.toArray$TTA(values);
});
