(function(){var P$=Clazz.newPackage("org.scijava.parse"),I$=[];
/*i*/var C$=Clazz.newInterface(P$, "ParseService", null, null, 'org.scijava.service.SciJavaService');

C$.$clinit$=2;
C$.$defaults$ = function(C$){

Clazz.newMeth(C$, 'parse$S', function (arg) {
return this.parse$S$Z(arg, true);
});
};})();
;Clazz.setTVer('3.2.6-v1');//Created 2020-01-04 11:21:28 Java2ScriptVisitor version 3.2.6-v1 net.sf.j2s.core.jar version 3.2.6-v1
