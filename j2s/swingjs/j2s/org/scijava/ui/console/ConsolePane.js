(function(){var P$=Clazz.newPackage("org.scijava.ui.console"),I$=[];
/*i*/var C$=Clazz.newInterface(P$, "ConsolePane", null, null, ['org.scijava.widget.UIComponent', 'org.scijava.console.OutputListener']);

C$.$clinit$=2;

C$.$static$ = function() {C$.$static$=0;
C$.NO_CONSOLE_PROPERTY="scijava.noConsole";
}

C$.$fields$=[[]
,['S',['NO_CONSOLE_PROPERTY']]]
})();
;Clazz.setTVer('3.2.6-v1');//Created 2020-01-04 11:21:32 Java2ScriptVisitor version 3.2.6-v1 net.sf.j2s.core.jar version 3.2.6-v1
