...
return Clazz.new_($I$(4,1).c$$java_util_Locale,[this.getLocale$()]);
});
...
palette[index++]=(this.metadata.PLTE_red[i]|0);
palette[index++]=(this.metadata.PLTE_green[i]|0);
palette[index++]=(this.metadata.PLTE_blue[i]|0);
}
cs.write$BA(palette);
cs.finish$();
}}, p$2);
...
var cs=Clazz.new_($I$(6,1).c$$I$javax_imageio_stream_ImageOutputStream,[C$.chunkType$S(type), this.stream]);
...
if (this.numBands == 1) {
for (var s=xOffset; s < numSamples; s+=xSkip) {
this.currRow[count++]=(this.scale0[samples[s]]|0);
if (icm_gray_alpha != null ) {
this.currRow[count++]=(this.scale0[icm_gray_alpha.getAlpha$I(255 & samples[s])]|0);
}}
} else {
for (var s=xOffset; s < numSamples; s+=xSkip) {
for (var b=0; b < this.numBands; b++) {
this.currRow[count++]=(this.scale[b][samples[s + b]]|0);
}
}
}break;
case 16:
for (var s=xOffset; s < numSamples; s+=xSkip) {
for (var b=0; b < this.numBands; b++) {
this.currRow[count++]=(this.scaleh[b][samples[s + b]]|0);
this.currRow[count++]=(this.scalel[b][samples[s + b]]|0);
}
}
break;
}
...
var imageBounds=Clazz.new_($I$(9,1).c$$I$I$I$I,[im.getMinX$(), im.getMinY$(), im.getWidth$(), im.getHeight$()]);
