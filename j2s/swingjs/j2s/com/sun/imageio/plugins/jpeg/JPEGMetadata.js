...
if (newGuy != null ) {
this.markerSequence.add$TE(newGuy);
if (false) {
newGuy.print$();
}newGuy=null;
}}
...
this.markerSequence.add$TE(Clazz.new_($I$(4,1).c$$javax_imageio_plugins_jpeg_JPEGQTableA,[jparam.getQTables$()]));
this.markerSequence.add$TE(Clazz.new_($I$(5,1).c$$javax_imageio_plugins_jpeg_JPEGHuffmanTableA$javax_imageio_plugins_jpeg_JPEGHuffmanTableA,[jparam.getDCHuffmanTables$(), jparam.getACHuffmanTables$()]));
} else {
this.markerSequence.add$TE(Clazz.new_($I$(4,1).c$$javax_imageio_plugins_jpeg_JPEGQTableA,[$I$(12).getDefaultQTables$()]));
this.markerSequence.add$TE(Clazz.new_($I$(5,1).c$$javax_imageio_plugins_jpeg_JPEGHuffmanTableA$javax_imageio_plugins_jpeg_JPEGHuffmanTableA,[$I$(12).getDefaultHuffmanTables$Z(true), $I$(12).getDefaultHuffmanTables$Z(false)]));
}if (!p$1.isConsistent.apply(this, [])) {
...
this.markerSequence.add$TE(jfif);
if (wantICC) {
try {
...
this.markerSequence.add$TE(Clazz.new_($I$(9,1).c$$I,[transform]));
}if (wantQTables) {
this.markerSequence.add$TE(Clazz.new_($I$(4,1).c$$F$Z,[quality, willSubsample]));
}if (wantHTables) {
this.markerSequence.add$TE(Clazz.new_($I$(5,1).c$$Z,[willSubsample]));
}this.markerSequence.add$TE(Clazz.new_($I$(3,1).c$$Z$Z$Z$BA$I,[wantProg, wantExtended, willSubsample, componentIDs, numComponents]));
if (!wantProg) {
this.markerSequence.add$TE(Clazz.new_($I$(11,1).c$$Z$BA$I,[willSubsample, componentIDs, numComponents]));
}if (!p$1.isConsistent.apply(this, [])) {
...
if (this.markerSequence == null ) {
return null;
}var retval=Clazz.new_($I$(1,1).c$$I,[this.markerSequence.size$()]);
var iter=this.markerSequence.iterator$();
while (iter.hasNext$()){
var seg=iter.next$();
retval.add$TE(seg.clone$());
}
return retval;
}, p$1);
...
this.markerSequence.add$I$TE(0, Clazz.new_($I$(7,1).c$$org_w3c_dom_Node,[jfifNode]));
}}}, p$1);
...
if (dqt != null ) {
dqt.tables.set$I$TE(tableIndex, dqt.getQtableFromNode$org_w3c_dom_Node(child));
} else {
dqt=oldDQTs.get$I(oldDQTs.size$() - 1);
dqt.tables.add$TE(dqt.getQtableFromNode$org_w3c_dom_Node(child));
}}
} else {
...
if (firstDHT != -1) {
this.markerSequence.add$I$TE(firstDHT, newGuy);
} else if (firstSOF != -1) {
this.markerSequence.add$I$TE(firstSOF, newGuy);
} else if (firstSOS != -1) {
this.markerSequence.add$I$TE(firstSOS, newGuy);
} else {
this.markerSequence.add$TE(newGuy);
}}}, p$1);
...
if (dht != null ) {
dht.tables.set$I$TE(tableIndex, dht.getHtableFromNode$org_w3c_dom_Node(child));
} else {
dht=oldDHTs.get$I(oldDHTs.size$() - 1);
dht.tables.add$TE(dht.getHtableFromNode$org_w3c_dom_Node(child));
}}
} else {
...
var firstSOS=p$1.findMarkerSegmentPosition$Class$Z.apply(this, [Clazz.getClass($I$(11)), true]);
if (lastDQT != -1) {
this.markerSequence.add$I$TE(lastDQT + 1, newGuy);
} else if (firstSOF != -1) {
this.markerSequence.add$I$TE(firstSOF, newGuy);
} else if (firstSOS != -1) {
this.markerSequence.add$I$TE(firstSOS, newGuy);
...
if (lastCOM != -1) {
this.markerSequence.add$I$TE(lastCOM + 1, newGuy);
} else if (hasJFIF) {
this.markerSequence.add$I$TE(1, newGuy);
} else if (firstAdobe != -1) {
this.markerSequence.add$I$TE(firstAdobe + 1, newGuy);
} else {
this.markerSequence.add$I$TE(0, newGuy);
}}, p$1);
...
this.markerSequence.add$I$TE(lastUnknown + 1, newGuy);
} else {
this.markerSequence.add$I$TE(0, newGuy);
}}, p$1);
...
this.markerSequence.add$I$TE(firstAdobe, newGuy);
} else {
this.markerSequence.add$I$TE(0, newGuy);
}}, p$1);
...
this.markerSequence.add$TE(Clazz.new_($I$(11,1).c$$org_w3c_dom_Node,[node]));
}}, p$1);
...
this.markerSequence.add$I$TE(0, Clazz.new_($I$(7,1)));
}if (wantAdobe) {
...
this.markerSequence.set$I$TE(this.markerSequence.indexOf$O(sof), Clazz.new_($I$(3,1).c$$Z$Z$Z$BA$I,[progressive, false, willSubsample, ids, numChannels]));
...
this.markerSequence.set$I$TE(this.markerSequence.indexOf$O(sos), Clazz.new_($I$(11,1).c$$Z$BA$I,[willSubsample, ids, numChannels]));
}}} else {
if (this.isStream) {
updateQtables=true;
updateHtables=true;
...
dqt.tables.add$TE(dqt.getChromaForLuma$com_sun_imageio_plugins_jpeg_DQTMarkerSegment_Qtable(table0));
}}}if (updateHtables) {
var tableSegments=Clazz.new_($I$(1,1));
for (var iter=this.markerSequence.iterator$(); iter.hasNext$(); ) {
var seg=iter.next$();
...
this.markerSequence.add$I$TE(0, jfif);
}}if (jfif != null ) {
var children=node.getChildNodes$();
for (var i=0; i < children.getLength$(); i++) {
var child=children.item$I(i);
...
var valueString=attrs.getNamedItem$S("value").getNodeValue$();
var value=Float.parseFloat$S(valueString);
var dpcm=(Math.round(1.0 / (value * 10.0))|0);
jfif.resUnits=2;
jfif.Xdensity=dpcm;
...
var oldFirstID=($b$[0] = (sof.componentSpecs[0].componentId|0), $b$[0]);
var newID=($b$[0] = (((oldFirstID > 1) ? "A" : String.fromCharCode(4)).$c()|0), $b$[0]);
newSpecs[numChannels - 1]=sof.getComponentSpec$B$I$I((newID|0), sof.componentSpecs[0].HsamplingFactor, sof.componentSpecs[0].QtableSelector);
sof.componentSpecs=newSpecs;
var newScanSpecs=Clazz.array($I$(20), [numChannels]);
for (var i=0; i < sos.componentSpecs.length; i++) {
newScanSpecs[i]=sos.componentSpecs[i];
}
newScanSpecs[numChannels - 1]=sos.getScanComponentSpec$B$I((newID|0), 0);
...
if (JPEGvariety.getChildNodes$().getLength$() != 0) {
this.markerSequence.add$TE(Clazz.new_($I$(7,1).c$$org_w3c_dom_Node,[JPEGvariety.getFirstChild$()]));
...
this.markerSequence.add$TE(Clazz.new_($I$(4,1).c$$org_w3c_dom_Node,[node]));
} else if (childName.equals$O("dht")) {
this.markerSequence.add$TE(Clazz.new_($I$(5,1).c$$org_w3c_dom_Node,[node]));
} else if (childName.equals$O("dri")) {
this.markerSequence.add$TE(Clazz.new_($I$(6,1).c$$org_w3c_dom_Node,[node]));
} else if (childName.equals$O("com")) {
this.markerSequence.add$TE(Clazz.new_($I$(10,1).c$$org_w3c_dom_Node,[node]));
} else if (childName.equals$O("app14Adobe")) {
this.markerSequence.add$TE(Clazz.new_($I$(9,1).c$$org_w3c_dom_Node,[node]));
} else if (childName.equals$O("unknown")) {
this.markerSequence.add$TE(Clazz.new_($I$(8,1).c$$org_w3c_dom_Node,[node]));
} else if (childName.equals$O("sof")) {
this.markerSequence.add$TE(Clazz.new_($I$(3,1).c$$org_w3c_dom_Node,[node]));
