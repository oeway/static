...
/*c*/var C$=Clazz.newClass(P$, "TFloatByteHash", null, 'gnu.trove.impl.hash.TPrimitiveHash');
...
C$.$fields$=[['Z',['consumeFreeSlot'],'B',['no_entry_value'],'F',['no_entry_key'],'O',['_set','float[]']]]
...
Clazz.newMeth(C$, 'c$$I$F$F$B', function (initialCapacity, loadFactor, no_entry_key, no_entry_value) {
;C$.superclazz.c$$I$F.apply(this,[initialCapacity, loadFactor]);C$.$init$.apply(this);
...
this._set=Clazz.array(Float.TYPE, [capacity]);
return capacity;
});

Clazz.newMeth(C$, 'contains$F', function (val) {
return this.index$F(val) >= 0;
});
...
Clazz.newMeth(C$, 'index$F', function (key) {
var hash;
var probe;
var index;
var length;
...
hash=$I$(1).hash$F(key) & 2147483647;
index=hash % length;
var state=($b$[0] = states[index], $b$[0]);
if (state == 0) return -1;
if (state == 1 && set[index] == key  ) return index;
return this.indexRehashed$F$I$I$B(key, index, hash, ($b$[0] = state, $b$[0]));
});

Clazz.newMeth(C$, 'indexRehashed$F$I$I$B', function (key, index, hash, state) {
...
Clazz.newMeth(C$, 'insertKey$F', function (val) {
var hash;
var index;
hash=$I$(1).hash$F(val) & 2147483647;
index=hash % this._states.length;
var state=($b$[0] = this._states[index], $b$[0]);
this.consumeFreeSlot=false;
if (state == 0) {
this.consumeFreeSlot=true;
this.insertKeyAt$I$F(index, val);
return index;
}if (state == 1 && this._set[index] == val  ) {
return -index - 1;
}return this.insertKeyRehash$F$I$I$B(val, index, hash, ($b$[0] = state, $b$[0]));
});

Clazz.newMeth(C$, 'insertKeyRehash$F$I$I$B', function (val, index, hash, state) {
var length=this._set.length;
var probe=1 + (hash % (length - 2));
var loopIndex=index;
var firstRemoved=-1;
...
if (state == 0) {
if (firstRemoved != -1) {
this.insertKeyAt$I$F(firstRemoved, val);
return firstRemoved;
} else {
this.consumeFreeSlot=true;
this.insertKeyAt$I$F(index, val);
...
Clazz.newMeth(C$, 'insertKeyAt$I$F', function (index, val) {
this._set[index]=val;
this._states[index]=(1|0);
});

Clazz.newMeth(C$, 'XinsertKey$F', function (key) {
var hash;
var probe;
var index;
var length;
