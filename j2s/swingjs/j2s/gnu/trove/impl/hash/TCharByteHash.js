...
/*c*/var C$=Clazz.newClass(P$, "TCharByteHash", null, 'gnu.trove.impl.hash.TPrimitiveHash');
...
C$.$fields$=[['Z',['consumeFreeSlot'],'B',['no_entry_value'],'C',['no_entry_key'],'O',['_set','char[]']]]

Clazz.newMeth(C$, 'c$', function () {
;C$.superclazz.c$.apply(this,[]);C$.$init$.apply(this);
this.no_entry_key="\u0000";
this.no_entry_value=($b$[0] = 0, $b$[0]);
}, 1);
...
Clazz.newMeth(C$, 'c$$I$F$C$B', function (initialCapacity, loadFactor, no_entry_key, no_entry_value) {
;C$.superclazz.c$$I$F.apply(this,[initialCapacity, loadFactor]);C$.$init$.apply(this);
...
this._set=Clazz.array(Character.TYPE, [capacity]);
return capacity;
});

Clazz.newMeth(C$, 'contains$C', function (val) {
return this.index$C(val) >= 0;
});
...
Clazz.newMeth(C$, 'index$C', function (key) {
var hash;
var probe;
var index;
var length;
...
hash=$I$(1).hash$I(key.$c()) & 2147483647;
index=hash % length;
var state=($b$[0] = states[index], $b$[0]);
if (state == 0) return -1;
if (state == 1 && set[index] == key ) return index;
return this.indexRehashed$C$I$I$B(key, index, hash, ($b$[0] = state, $b$[0]));
});

Clazz.newMeth(C$, 'indexRehashed$C$I$I$B', function (key, index, hash, state) {
...
Clazz.newMeth(C$, 'insertKey$C', function (val) {
var hash;
var index;
hash=$I$(1).hash$I(val.$c()) & 2147483647;
index=hash % this._states.length;
var state=($b$[0] = this._states[index], $b$[0]);
this.consumeFreeSlot=false;
if (state == 0) {
this.consumeFreeSlot=true;
this.insertKeyAt$I$C(index, val);
return index;
}if (state == 1 && this._set[index] == val ) {
return -index - 1;
}return this.insertKeyRehash$C$I$I$B(val, index, hash, ($b$[0] = state, $b$[0]));
});

Clazz.newMeth(C$, 'insertKeyRehash$C$I$I$B', function (val, index, hash, state) {
var length=this._set.length;
var probe=1 + (hash % (length - 2));
var loopIndex=index;
var firstRemoved=-1;
...
if (state == 0) {
if (firstRemoved != -1) {
this.insertKeyAt$I$C(firstRemoved, val);
return firstRemoved;
} else {
this.consumeFreeSlot=true;
this.insertKeyAt$I$C(index, val);
...
Clazz.newMeth(C$, 'insertKeyAt$I$C', function (index, val) {
this._set[index]=val;
this._states[index]=(1|0);
});

Clazz.newMeth(C$, 'XinsertKey$C', function (key) {
var hash;
var probe;
var index;
var length;
