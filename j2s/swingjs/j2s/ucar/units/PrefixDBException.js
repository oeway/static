(function(){var P$=Clazz.newPackage("ucar.units"),I$=[];
/*c*/var C$=Clazz.newClass(P$, "PrefixDBException", null, 'ucar.units.UnitException');

C$.$clinit$=2;

Clazz.newMeth(C$, '$init$', function () {
}, 1);

Clazz.newMeth(C$, 'c$', function () {
;C$.superclazz.c$$S.apply(this,["Prefix database exception"]);C$.$init$.apply(this);
}, 1);

Clazz.newMeth(C$, 'c$$S', function (message) {
;C$.superclazz.c$$S.apply(this,["Prefix database exception: " + message]);C$.$init$.apply(this);
}, 1);

Clazz.newMeth(C$, 'c$$Exception', function (e) {
C$.c$$S$Exception.apply(this, ["Prefix database exception", e]);
}, 1);

Clazz.newMeth(C$, 'c$$S$Exception', function (message, e) {
;C$.superclazz.c$$S$Exception.apply(this,[message, e]);C$.$init$.apply(this);
}, 1);
})();
;Clazz.setTVer('3.2.6-v1');//Created 2020-01-04 11:20:32 Java2ScriptVisitor version 3.2.6-v1 net.sf.j2s.core.jar version 3.2.6-v1
