(function(){var P$=Clazz.newPackage("java.security"),I$=[[0,'sun.security.util.Debug','java.security.AccessController','java.util.concurrent.ConcurrentHashMap','java.util.Properties','java.io.FileInputStream','java.io.BufferedInputStream','sun.security.util.PropertyExpander','java.io.File','java.net.URL','sun.security.jca.Providers',['java.security.Security','.ProviderProperty'],'sun.security.jca.ProviderList','java.util.Hashtable','java.util.LinkedHashSet','java.security.Provider','AssertionError','sun.security.jca.GetInstance','java.security.SecurityPermission','java.util.Collections','java.util.HashSet','java.util.Locale']],$I$=function(i,n){return(i=(I$[i]||(I$[i]=Clazz.load(I$[0][i])))),!n&&i.$load$&&Clazz.load(i,2),i};
...
P$.Security.initialize$();
return null;
});
})()
), Clazz.new_(P$.Security$1.$init$, [this, null])));
};
C$.spiMap=Clazz.new_(1,{K:"String",V:"Class"},$I$(3,1));
}
...
var is=null;
try {
var fis=Clazz.new_($I$(5,1).c$$java_io_File,[propFile]);
is=Clazz.new_($I$(6,1).c$$java_io_InputStream,[fis]);
...
try {
var propURL;
extraPropFile=$I$(7).expand$S(extraPropFile);
propFile=Clazz.new_($I$(8,1).c$$S,[extraPropFile]);
if (propFile.exists$()) {
propURL=Clazz.new_($I$(9,1).c$$S,["file:" + propFile.getCanonicalPath$()]);
} else {
propURL=Clazz.new_($I$(9,1).c$$S,[extraPropFile]);
}bis=Clazz.new_($I$(6,1).c$$java_io_InputStream,[propURL.openStream$()]);
...
C$.props.put$TK$TV("security.provider.1", "sun.security.provider.Sun");
C$.props.put$TK$TV("security.provider.2", "sun.security.rsa.SunRsaSign");
C$.props.put$TK$TV("security.provider.3", "com.sun.net.ssl.internal.ssl.Provider");
C$.props.put$TK$TV("security.provider.4", "com.sun.crypto.provider.SunJCE");
C$.props.put$TK$TV("security.provider.5", "sun.security.jgss.SunProvider");
C$.props.put$TK$TV("security.provider.6", "com.sun.security.sasl.Provider");
}, 1);
...
var sep=$I$(8).separator;
return Clazz.new_($I$(8,1).c$$S,[System.getProperty$S("java.home") + sep + "lib" + sep + "security" + sep + filename ]);
}, 1);

Clazz.newMeth(C$, 'getProviderProperty$S', function (key) {
var entry=null;
var providers=$I$(10).getProviderList$().providers$();
...
var newEntry=Clazz.new_($I$(11,1));
...
var list=$I$(10).getFullProviderList$();
var newList=$I$(12).insertAt$sun_security_jca_ProviderList$java_security_Provider$I(list, provider, position - 1);
if (list === newList ) {
return -1;
}$I$(10).setProviderList$sun_security_jca_ProviderList(newList);
...
var list=$I$(10).getFullProviderList$();
var newList=$I$(12).remove$sun_security_jca_ProviderList$S(list, name);
$I$(10).setProviderList$sun_security_jca_ProviderList(newList);
}, 1);

Clazz.newMeth(C$, 'getProviders$', function () {
return $I$(10).getFullProviderList$().toArray$();
}, 1);

Clazz.newMeth(C$, 'getProvider$S', function (name) {
return $I$(10).getProviderList$().getProvider$S(name);
}, 1);
...
}var hashtableFilter=Clazz.new_(1,{K:"String",V:"String"},$I$(13,1).c$$I,[1]);
hashtableFilter.put$TK$TV(key, value);
...
var candidates=Clazz.new_(1,{E:"java.security.Provider"},$I$(14,1).c$$I,[5]);
...
var result=Clazz.array($I$(15), [candidatesArray.length]);
...
C$.spiMap.put$TK$TV(type, clazz);
return clazz;
} catch (e) {
if (Clazz.exceptionOf(e,"ClassNotFoundException")){
throw Clazz.new_($I$(16,1).c$$S$Throwable,["Spi class not found", e]);
} else {
throw e;
}
}
...
if (provider == null ) {
return $I$(17).getInstance$S$Class$S(type, C$.getSpiClass$S(type), algorithm).toArray$();
} else {
return $I$(17).getInstance$S$Class$S$S(type, C$.getSpiClass$S(type), algorithm, provider).toArray$();
}}, 1);

Clazz.newMeth(C$, 'getImpl$S$S$S$O', function (algorithm, type, provider, params) {
if (provider == null ) {
return $I$(17).getInstance$S$Class$S$O(type, C$.getSpiClass$S(type), algorithm, params).toArray$();
} else {
return $I$(17).getInstance$S$Class$S$O$S(type, C$.getSpiClass$S(type), algorithm, params, provider).toArray$();
}}, 1);

Clazz.newMeth(C$, 'getImpl$S$S$java_security_Provider', function (algorithm, type, provider) {
return $I$(17).getInstance$S$Class$S$java_security_Provider(type, C$.getSpiClass$S(type), algorithm, provider).toArray$();
}, 1);

Clazz.newMeth(C$, 'getImpl$S$S$java_security_Provider$O', function (algorithm, type, provider, params) {
return $I$(17).getInstance$S$Class$S$O$java_security_Provider(type, C$.getSpiClass$S(type), algorithm, params, provider).toArray$();
}, 1);

Clazz.newMeth(C$, 'getProperty$S', function (key) {
var sm=System.getSecurityManager$();
if (sm != null ) {
sm.checkPermission$java_security_Permission(Clazz.new_($I$(18,1).c$$S,["getProperty." + key]));
...
candidates.add$TE(allProviders[i]);
}}
return candidates;
}, 1);
...
return $I$(19).emptySet$();
}var result=Clazz.new_(1,{E:"String"},$I$(20,1));
var providers=C$.getProviders$();
for (var i=0; i < providers.length; i++) {
for (var e=providers[i].keys$(); e.hasMoreElements$(); ) {
var currentKey=(e.nextElement$()).toUpperCase$java_util_Locale($I$(21).ENGLISH);
if (currentKey.startsWith$S(serviceName.toUpperCase$java_util_Locale($I$(21).ENGLISH))) {
if (currentKey.indexOf$S(" ") < 0) {
result.add$TE(currentKey.substring$I(serviceName.length$() + 1));
}}}
}
return $I$(19).unmodifiableSet$java_util_Set(result);
}, 1);
;
