...
var set=Clazz.new_(1,{E:"java.nio.file.OpenOption"},$I$(6,1).c$$I,[options.length]);
...
throw Clazz.new_($I$(21,1).c$$O,["Should not get here"]);
}
return followLinks;
}, 1);
...
try {
var walker=Clazz.new_($I$(25,1).c$$java_util_Collection$I,[options, maxDepth]);
/*try*/ {
...
try{walker&&walker.close$&&walker.close$()}catch(_){}}
return start;
}, 1);
...
var reader=Clazz.new_($I$(27,1).c$$java_io_InputStream$S,[C$.newInputStream$java_nio_file_Path$java_nio_file_OpenOptionA(path, []), decoder]);
return Clazz.new_($I$(28,1).c$$java_io_Reader,[reader]);
}, 1);
...
var writer=Clazz.new_($I$(30,1).c$$java_io_OutputStream$java_nio_charset_CharsetEncoder,[C$.newOutputStream$java_nio_file_Path$java_nio_file_OpenOptionA(path, options), encoder]);
return Clazz.new_($I$(31,1).c$$java_io_Writer,[writer]);
}, 1);
...
try {
var $in=C$.newInputStream$java_nio_file_Path$java_nio_file_OpenOptionA(source, []);
/*try*/ {
return C$.copy$java_io_InputStream$java_io_OutputStream($in, out);
}
}finally{/*res*/
...
if (capacity == 2147483639) throw Clazz.new_($I$(33,1).c$$S,["Required array size too large"]);
capacity=2147483639;
...
try {
var sbc=C$.newByteChannel$java_nio_file_Path$java_nio_file_OpenOptionA(path, []);
var $in=$I$(35).newInputStream$java_nio_channels_ReadableByteChannel(sbc);
/*try*/ {
var size=sbc.size$();
if (size > 2147483639) throw Clazz.new_($I$(33,1).c$$S,["Required array size too large"]);
return C$.read$java_io_InputStream$I($in, (size|0));
}
}finally{/*res*/
try{$in&&$in.close$&&$in.close$()}catch(_){}
try{sbc&&sbc.close$&&sbc.close$()}catch(_){}}
}, 1);
...
try{reader&&reader.close$&&reader.close$()}catch(_){}}
}, 1);
...
var out=C$.newOutputStream$java_nio_file_Path$java_nio_file_OpenOptionA(path, options);
try {
var writer=Clazz.new_($I$(31,1).c$$java_io_Writer,[Clazz.new_($I$(30,1).c$$java_io_OutputStream$java_nio_charset_CharsetEncoder,[out, encoder])]);
/*try*/ {
...
try{writer&&writer.close$&&writer.close$()}catch(_){}}
return path;
}, 1);
...
var iterator=Clazz.new_($I$(38,1).c$$java_nio_file_Path$I$java_nio_file_FileVisitOptionA,[start, maxDepth, options]);
try {
return $I$(36).stream$java_util_Spliterator$Z($I$(37).spliteratorUnknownSize$java_util_Iterator$I(iterator, 1), false).onClose$Runnable((function($$){((
...
), Clazz.new_(1,{T:"? super java.nio.file.FileTreeWalker.Event",R:"? extends java.nio.file.Path"},P$.Files$lambda3.$init$, [this, null]))))));
} catch (e) {
if (Clazz.exceptionOf(e,"Error") || Clazz.exceptionOf(e,"RuntimeException")){
iterator.close$();
throw e;
...
), Clazz.new_(1,{T:"? super java.nio.file.FileTreeWalker.Event"},P$.Files$lambda5.$init$, [this, {matcher:matcher}]))).map$java_util_function_Function((P$.Files$lambda6$||(P$.Files$lambda6$=(((P$.Files$lambda6||
...
), Clazz.new_(1,{T:"? super java.nio.file.FileTreeWalker.Event",R:"? extends java.nio.file.Path"},P$.Files$lambda6.$init$, [this, null]))))));
} catch (e) {
if (Clazz.exceptionOf(e,"Error") || Clazz.exceptionOf(e,"RuntimeException")){
iterator.close$();
throw e;
...
Clazz.newMeth(C$, 'run$', function () {
var list=Clazz.new_(1,{E:"java.nio.file.spi.FileTypeDetector"},$I$(3,1));
