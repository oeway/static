...
$I$(2).requireNonNull$TT$S(offsetId, "offsetId");
...
var totalSecs=new Integer(totalSeconds);
var result=C$.SECONDS_CACHE.get$O(totalSecs);
if (result == null ) {
result=Clazz.new_(C$.c$$I,[totalSeconds]);
C$.SECONDS_CACHE.putIfAbsent$TK$TV(totalSecs, result);
result=C$.SECONDS_CACHE.get$O(totalSecs);
C$.ID_CACHE.putIfAbsent$TK$TV(result.getId$(), result);
}return result;
} else {
...
Clazz.newMeth(C$, ['compareTo$java_time_ZoneOffset','compareTo$','compareTo$TT'], function (other) {
...
return Clazz.new_($I$(7,1).c$$B$O,[($b$[0] = 8, $b$[0]), this]);
}, p$1);
