(function(){var P$=Clazz.newPackage("java.time.zone"),p$1={},I$=[[0,'java.time.zone.ZoneOffsetTransitionRule','java.time.LocalDateTime','java.util.concurrent.ConcurrentHashMap','java.util.Objects','java.time.ZoneOffset','java.util.ArrayList','java.time.zone.ZoneOffsetTransition','java.time.zone.Ser','java.util.Arrays','java.util.Collections','java.time.Duration','java.time.LocalDate']],$I$=function(i,n){return(i=(I$[i]||(I$[i]=Clazz.load(I$[0][i])))),!n&&i.$load$&&Clazz.load(i,2),i};
/*c*/var C$=Clazz.newClass(P$, "ZoneRules", null, null, 'java.io.Serializable');
C$.EMPTY_LONG_ARRAY=null;
C$.EMPTY_LASTRULES=null;
C$.EMPTY_LDT_ARRAY=null;
...
this.standardOffsets=null;
this.savingsInstantTransitions=null;
this.savingsLocalTransitions=null;
this.wallOffsets=null;
this.lastRules=null;
...
$I$(4).requireNonNull$TT$S(baseStandardOffset, "baseStandardOffset");
$I$(4).requireNonNull$TT$S(baseWallOffset, "baseWallOffset");
$I$(4).requireNonNull$TT$S(standardOffsetTransitionList, "standardOffsetTransitionList");
$I$(4).requireNonNull$TT$S(transitionList, "transitionList");
$I$(4).requireNonNull$TT$S(lastRules, "lastRules");
...
localTransitionOffsetList.add$TE(baseWallOffset);
for (var trans, $trans = transitionList.iterator$(); $trans.hasNext$()&&((trans=($trans.next$())),1);) {
if (trans.isGap$()) {
localTransitionList.add$TE(trans.getDateTimeBefore$());
localTransitionList.add$TE(trans.getDateTimeAfter$());
} else {
localTransitionList.add$TE(trans.getDateTimeAfter$());
localTransitionList.add$TE(trans.getDateTimeBefore$());
}localTransitionOffsetList.add$TE(trans.getOffsetAfter$());
}
this.savingsLocalTransitions=localTransitionList.toArray$TTA(Clazz.array($I$(2), [localTransitionList.size$()]));
this.wallOffsets=localTransitionOffsetList.toArray$TTA(Clazz.array($I$(5), [localTransitionOffsetList.size$()]));
...
}this.lastRules=lastRules.toArray$TTA(Clazz.array($I$(1), [lastRules.size$()]));
}, 1);
...
return Clazz.new_($I$(8,1).c$$B$O,[($b$[0] = 1, $b$[0]), this]);
}, p$1);
...
}return $I$(10).singletonList$TT(info);
});
...
if (year < 2100) {
this.lastRulesCache.putIfAbsent$TK$TV(yearObj, transArray);
}return transArray;
}, p$1);
...
list.add$TE(Clazz.new_($I$(7,1).c$$J$java_time_ZoneOffset$java_time_ZoneOffset,[this.savingsInstantTransitions[i], this.wallOffsets[i], this.wallOffsets[i + 1]]));
}
return $I$(10).unmodifiableList$java_util_List(list);
});

Clazz.newMeth(C$, 'getTransitionRules$', function () {
return $I$(10).unmodifiableList$java_util_List($I$(9).asList$TTA(this.lastRules));
});
