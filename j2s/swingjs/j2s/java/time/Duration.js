...
C$.BI_NANOS_PER_SECOND=$I$(4).valueOf$J(1000000000);
C$.PATTERN=$I$(5).compile$S$I("([-+]?)P(?:([-+]?[0-9]+)D)?(T(?:([-+]?[0-9]+)H)?(?:([-+]?[0-9]+)M)?(?:([-+]?[0-9]+)(?:[.,]([0-9]{0,9}))?S)?)?", 2);
}
...
$I$(6).requireNonNull$TT$S(amount, "amount");
var duration=C$.ZERO;
...
$I$(6).requireNonNull$TT$S(text, "text");
...
try {
return C$.ofNanos$J(startInclusive.until$java_time_temporal_Temporal$java_time_temporal_TemporalUnit(endExclusive, $I$(3).NANOS));
} catch (ex) {
if (Clazz.exceptionOf(ex,"java.time.DateTimeException") || Clazz.exceptionOf(ex,"ArithmeticException")){
var secs=startInclusive.until$java_time_temporal_Temporal$java_time_temporal_TemporalUnit(endExclusive, $I$(3).SECONDS);
var nanos;
try {
nanos=endExclusive.getLong$java_time_temporal_TemporalField($I$(7).NANO_OF_SECOND) - startInclusive.getLong$java_time_temporal_TemporalField($I$(7).NANO_OF_SECOND);
if (secs > 0 && nanos < 0 ) {
secs++;
...
$I$(7).NANO_OF_SECOND.checkValidIntValue$J(nanoOfSecond);
...
$I$(6).requireNonNull$TT$S(unit, "unit");
if (unit === $I$(3).DAYS ) {
...
case $I$(3).MICROS:
return this.plusSeconds$J(((amountToAdd/(1000000000)|0)) * 1000).plusNanos$J((amountToAdd % (1000000000)) * 1000);
case $I$(3).MILLIS:
...
return this;
}return C$.create$java_math_BigDecimal(p$1.toSeconds.apply(this, []).multiply$java_math_BigDecimal($I$(9).valueOf$J(multiplicand)));
});
...
return this;
}return C$.create$java_math_BigDecimal(p$1.toSeconds.apply(this, []).divide$java_math_BigDecimal$java_math_RoundingMode($I$(9).valueOf$J(divisor), $I$(10).DOWN));
});

Clazz.newMeth(C$, 'toSeconds', function () {
return $I$(9).valueOf$J(this.seconds).add$java_math_BigDecimal($I$(9).valueOf$J$I(this.nanos, 9));
}, p$1);
...
if (this.seconds != 0) {
temporal=temporal.plus$J$java_time_temporal_TemporalUnit(this.seconds, $I$(3).SECONDS);
}if (this.nanos != 0) {
temporal=temporal.plus$J$java_time_temporal_TemporalUnit(this.nanos, $I$(3).NANOS);
}return temporal;
});

Clazz.newMeth(C$, 'subtractFrom$java_time_temporal_Temporal', function (temporal) {
if (this.seconds != 0) {
temporal=temporal.minus$J$java_time_temporal_TemporalUnit(this.seconds, $I$(3).SECONDS);
}if (this.nanos != 0) {
temporal=temporal.minus$J$java_time_temporal_TemporalUnit(this.nanos, $I$(3).NANOS);
}return temporal;
});
...
Clazz.newMeth(C$, ['compareTo$java_time_Duration','compareTo$','compareTo$TT'], function (otherDuration) {
...
if (this === C$.ZERO ) {
return "PT0S";
}var hours=(this.seconds/3600|0);
var minutes=((((this.seconds % 3600)/60|0))|0);
var secs=((this.seconds % 60)|0);
var buf=Clazz.new_($I$(11,1).c$$I,[24]);
buf.append$S("PT");
if (hours != 0) {
...
return Clazz.new_($I$(12,1).c$$B$O,[($b$[0] = 1, $b$[0]), this]);
}, p$1);
...
C$.$static$ = function() {C$.$static$=0;
C$.UNITS=$I$(1).unmodifiableList$java_util_List($I$(2).asList$TTA([$I$(3).SECONDS, $I$(3).NANOS]));
}
