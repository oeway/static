(function(){var P$=Clazz.newPackage("java.util.stream"),p$1={},p$2={},I$=[[0,'java.util.Objects','java.util.stream.SpinedBuffer','java.util.stream.StreamSupport',['java.util.stream.SpinedBuffer','.OfInt'],['java.util.stream.SpinedBuffer','.OfLong'],['java.util.stream.SpinedBuffer','.OfDouble'],'Error']],$I$=function(i,n){return(i=(I$[i]||(I$[i]=Clazz.load(I$[0][i])))),!n&&i.$load$&&Clazz.load(i,2),i};
...
if (this.buffer == null ) {
this.buffer=Clazz.new_(1,{E:"Object"},$I$(2,1));
...
C$.$fields$=[['L',['first'],'O',['buffer','java.util.stream.SpinedBuffer.OfLong']]]
...
;C$.superclazz.c$$TT_SPLITR$TT_SPLITR.apply(this,[aSpliterator, bSpliterator]);C$.$init$.apply(this);
}, 1);
...
var hasNext;
if (this.beforeSplit) {
hasNext=this.aSpliterator.tryAdvance$TC(action);
if (!hasNext) {
this.beforeSplit=false;
hasNext=this.bSpliterator.tryAdvance$TC(action);
}} else hasNext=this.bSpliterator.tryAdvance$TC(action);
return hasNext;
});

Clazz.newMeth(C$, ['forEachRemaining$TT_CONS','forEachRemaining$TC'], function (action) {
if (this.beforeSplit) this.aSpliterator.forEachRemaining$TC(action);
this.bSpliterator.forEachRemaining$TC(action);
});
