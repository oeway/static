(function(){var P$=Clazz.newPackage("java.util.concurrent.atomic"),I$=[[0,'sun.misc.Unsafe','Error','Runtime','Thread','java.util.concurrent.ThreadLocalRandom',['java.util.concurrent.atomic.Striped64','.Cell']]],$I$=function(i,n){return(i=(I$[i]||(I$[i]=Clazz.load(I$[0][i])))),!n&&i.$load$&&Clazz.load(i,2),i};
...
C$.BASE=C$.UNSAFE.objectFieldOffset$reflect_Field(sk.getDeclaredField$S("base"));
C$.CELLSBUSY=C$.UNSAFE.objectFieldOffset$reflect_Field(sk.getDeclaredField$S("cellsBusy"));
...
C$.$fields$=[['I',['cellsBusy'],'L',['base'],'O',['cells','java.util.concurrent.atomic.Striped64.Cell[]']]
,['I',['NCPU'],'L',['BASE','CELLSBUSY','PROBE'],'O',['UNSAFE','sun.misc.Unsafe']]]
...
continue;
 ...
 else if (a.cas$J$J(v=a.value, ((fn == null ) ? v + x : fn.applyAsLong$(v, x)))) break;
...
if (init) break;
} else if (this.casBase$J$J(v=this.base, ((fn == null ) ? v + x : fn.applyAsLong$(v, x)))) break;
}
});
...
if ((a=as[(n - 1) & h]) == null ) {
if (this.cellsBusy == 0) {
var r=Clazz.new_($I$(6,1).c$$J,[Double.doubleToRawLongBits$D(x)]);
if (this.cellsBusy == 0 && this.casCellsBusy$() ) {
var created=false;
try {
var rs;
...
continue;
 ...
 else if (a.cas$J$J(v=a.value, ((fn == null ) ? Double.doubleToRawLongBits$D(Double.longBitsToDouble$J(v) + x) : Double.doubleToRawLongBits$D(fn.applyAsDouble$(Double.longBitsToDouble$J(v), x))))) break;
 else if (n >= C$.NCPU || this.cells !== as  ) collide=false;
 else if (!collide) collide=true;
 else if (this.cellsBusy == 0 && this.casCellsBusy$() ) {
try {
...
if (this.cells === as ) {
var rs=Clazz.array($I$(6), [2]);
rs[h & 1]=Clazz.new_($I$(6,1).c$$J,[Double.doubleToRawLongBits$D(x)]);
this.cells=rs;
init=true;
}} finally {
this.cellsBusy=0;
...
if (init) break;
} else if (this.casBase$J$J(v=this.base, ((fn == null ) ? Double.doubleToRawLongBits$D(Double.longBitsToDouble$J(v) + x) : Double.doubleToRawLongBits$D(fn.applyAsDouble$(Double.longBitsToDouble$J(v), x))))) break;
}
});
;
...
C$.valueOffset=C$.UNSAFE.objectFieldOffset$reflect_Field(ak.getDeclaredField$S("value"));
} catch (e) {
if (Clazz.exceptionOf(e,"Exception")){
throw Clazz.new_($I$(2,1).c$$Throwable,[e]);
} else {
