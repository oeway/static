(function(){var P$=Clazz.newPackage("java.util.prefs"),I$=[[0,'org.xml.sax.InputSource','java.io.StringReader','java.util.ArrayList','java.util.prefs.Preferences','javax.xml.parsers.DocumentBuilderFactory','AssertionError',['java.util.prefs.XmlSupport','.Resolver'],['java.util.prefs.XmlSupport','.EH'],'javax.xml.transform.TransformerFactory','javax.xml.transform.dom.DOMSource','javax.xml.transform.stream.StreamResult','java.io.BufferedWriter','java.io.OutputStreamWriter']],$I$=function(i,n){return(i=(I$[i]||(I$[i]=Clazz.load(I$[0][i])))),!n&&i.$load$&&Clazz.load(i,2),i};
...
var ancestors=Clazz.new_(1,{E:"java.util.prefs.Preferences"},$I$(3,1));
for (var kid=p, dad=kid.parent$(); dad != null ; kid=dad, dad=kid.parent$()) {
ancestors.add$TE(kid);
}
var e=xmlRoot;
...
t.transform$javax_xml_transform_Source$javax_xml_transform_Result(Clazz.new_($I$(10,1).c$$org_w3c_dom_Node,[doc]), Clazz.new_($I$(11,1).c$$java_io_Writer,[Clazz.new_($I$(12,1).c$$java_io_Writer,[Clazz.new_($I$(13,1).c$$java_io_OutputStream$S,[out, "UTF-8"])])]));
} catch (e) {
...
for (var i=0, numEntries=entries.getLength$(); i < numEntries; i++) {
var entry=entries.item$I(i);
m.put$TK$TV(entry.getAttribute$S("key"), entry.getAttribute$S("value"));
}
} catch (e) {
if (Clazz.exceptionOf(e,"org.xml.sax.SAXException")){
throw Clazz.new_(Clazz.load('java.util.prefs.InvalidPreferencesFormatException').c$$Throwable,[e]);
...
Clazz.newMeth(C$, ['resolveEntity$S$S','resolveEntity$'], function (pid, sid) {
if (sid.equals$O("http://java.sun.com/dtd/preferences.dtd")) {
var is;
is=Clazz.new_($I$(1,1).c$$java_io_Reader,[Clazz.new_($I$(2,1).c$$S,["<?xml version=\"1.0\" encoding=\"UTF-8\"?><!-- DTD for preferences --><!ELEMENT preferences (root) ><!ATTLIST preferences EXTERNAL_XML_VERSION CDATA \"0.0\"  ><!ELEMENT root (map, node*) ><!ATTLIST root          type (system|user) #REQUIRED ><!ELEMENT node (map, node*) ><!ATTLIST node          name CDATA #REQUIRED ><!ELEMENT map (entry*) ><!ATTLIST map  MAP_XML_VERSION CDATA \"0.0\"  ><!ELEMENT entry EMPTY ><!ATTLIST entry          key CDATA #REQUIRED          value CDATA #REQUIRED >"])]);
