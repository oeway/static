...
C$.eventQueue=Clazz.new_(1,{E:"java.util.EventObject"},$I$(2,1));
C$.eventDispatchThread=null;
}

Clazz.newMeth(C$, '$init$', function () {
this.newNode=false;
this.kidCache=Clazz.new_(1,{K:"String",V:"java.util.prefs.AbstractPreferences"},$I$(3,1));
this.removed=false;
...
$I$(6).requireNonNull$TT$S(key, "Specified key cannot be null");
{
if (this.removed) throw Clazz.new_(Clazz.load('IllegalStateException').c$$S,["Node has been removed."]);
...
{
if (this.removed) throw Clazz.new_(Clazz.load('IllegalStateException').c$$S,["Node has been removed."]);
var s=Clazz.new_(1,{E:"String"},$I$(8,1).c$$java_util_Collection,[this.kidCache.keySet$()]);
for (var kid, $kid = 0, $$kid = this.childrenNamesSpi$(); $kid<$$kid.length&&((kid=($$kid[$kid])),1);$kid++) s.add$TE(kid);

return s.toArray$TTA(C$.EMPTY_STRING_ARRAY);
}});

Clazz.newMeth(C$, 'cachedChildren$', function () {
return this.kidCache.values$().toArray$TTA(C$.EMPTY_ABSTRACT_PREFS_ARRAY);
});
...
}return p$1.node$java_util_StringTokenizer.apply(this.root, [Clazz.new_($I$(9,1).c$$S$S$Z,[path.substring$I(1), "/", true])]);
});
...
this.kidCache.put$TK$TV(token, child);
...
}return p$1.nodeExists$java_util_StringTokenizer.apply(this.root, [Clazz.new_($I$(9,1).c$$S$S$Z,[path.substring$I(1), "/", true])]);
});
...
for (var i=0; i < kidNames.length; i++) if (!this.kidCache.containsKey$O(kidNames[i])) this.kidCache.put$TK$TV(kidNames[i], this.childSpi$S(kidNames[i]));
...
return new Boolean(this.b$['java.util.prefs.AbstractPreferences'].root === $I$(11).userRoot$() );
});
})()
...
{
C$.eventQueue.add$TE(Clazz.new_($I$(13,1).c$$java_util_prefs_Preferences$S$S,[this, key, newValue]));
C$.eventQueue.notify$();
}}}, p$1);
...
{
C$.eventQueue.add$TE(Clazz.new_($I$(14,1).c$$java_util_prefs_Preferences$java_util_prefs_Preferences, [this, null, this, child]));
C$.eventQueue.notify$();
}}}, p$1);
...
if (this.nodeListeners.length != 0) {
{
C$.eventQueue.add$TE(Clazz.new_($I$(15,1).c$$java_util_prefs_Preferences$java_util_prefs_Preferences, [this, null, this, child]));
C$.eventQueue.notify$();
}}}, p$1);
...
for (var i=0; i < listeners.length; i++) listeners[i].preferenceChange$(pce);
